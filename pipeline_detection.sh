...
bwa mem -T 10 -t $THREAD_COUNT ${fa} ${file}_1.fastq.gz ${file}_2.fastq.gz -o ${dir_detect}/align.sam
...
perl ${scriptdir}/CIRI-AS.pl --sam ${dir_detect}/align.sam --ciri ${dir_detect}/ciri.report --out ${dir_detect}/as --ref_file ${fa} --anno ${gtf} --output_all yes

# Generate full_ro1_align.txt and full_ro1.fq files
/usr/bin/java -jar ${scriptdir}/CIRI-full.jar RO1 -1 ${file}_1.fastq -2 ${file}_2.fastq -o ${dir_detect}/full -t $THREAD_COUNT 
...
/usr/bin/java -jar ${scriptdir}/CIRI-full.jar RO2 -r ${fa} -s ${dir_detect}/full_ro1.sam -l 150 -o ${dir_detect}/full 

# Generate full_merge_circRNA_detail.anno file
/usr/bin/java -jar ${scriptdir}CIRI-full.jar Merge -r ${fa} -a ${gtf} -c ${dir_detect}/ciri.report -as ${dir_detect}/as_jav.list -ro ${dir_detect}/full_ro2_info.list -o ${dir_detect}/full

# Visualization
/usr/bin/java -jar  ${scriptdir}/CIRI-vis.jar -i ${dir_detect}/full_merge_circRNA_detail.anno -l ${dir_detect}/as_library_length.list -d ${dir_vis} -r ${fa} -min 1
